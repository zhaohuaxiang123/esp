...
{
public:
    CTouchAdapter(xpt_conf_t * xpt_conf, int w, int h, float xfactor = 0.071006,
            float yfactor = 0.086557, int xoffset = -24, int yoffset = -13):
                CXpt2046(xpt_conf, w, h, xfactor,
                        yfactor, xoffset, yoffset)
    {
...
void board_touch_init()
{
    ...
    if(xpt == NULL){
        xpt= new CTouchAdapter(&xpt_conf, UGFX_DRIVER_SCREEN_WIDTH, UGFX_DRIVER_SCREEN_HEIGHT);
    }
    xpt->set_rotation(0);
}
