...
void ir_learn_test()
{
    ...
    ir_learn_result_t *ir_data = calloc(1, sizeof(ir_learn_result_t) + IR_LEARN_BUF_LEN * 2);
    ir_data->message_len = IR_LEARN_BUF_LEN;

    ir_learn_handle_t ir_handle = ir_learn_create(IR_LEARN_GPIO);
    ir_learn_start(ir_handle);

    // wait 60 second
    ir_learn_wait_finish(ir_handle, 60 * 1000 / portTICK_PERIOD_MS);

    ir_learn_stop(ir_handle);
    ir_learn_get_result(ir_handle, ir_data, true);
    ...
    ir_learn_send(RECV_RMT_TX_CHN, ir_data, 100 / portTICK_PERIOD_MS);
    ir_learn_send_deinit(RECV_RMT_TX_CHN);

    ir_learn_delete(ir_handle);
    free(ir_data);
...
TEST_CASE("IR learn test", "[IR][Learn]")
{
    ir_learn_test();
}
